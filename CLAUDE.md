...
2. **Unit Tests** - Test business logic with mocked dependencies
...
pnpm test:unit          # Test business logic (mocked services)
...
- **Integration Tests**: Mock only external APIs, use real Redis
- **Connection Tests**: Never mock - test real connections
...
describe('Error Handling', () => {
  ...
  it('should validate input and throw specific errors', () => {
    const invalidInput = { email: 'not-an-email' };
    
    expect(() => validateInput(invalidInput))
...
const customUser = createMockUser({ email: 'custom@test.com' });
```

#### Test Fixtures
```typescript
// __tests__/fixtures/notifications.json
{
  "maintenanceNotification": {
    ...
    "payload": {
      ...
      "message": "Scheduled HVAC maintenance"
    }
  }
}
...
const notification = fixtures.maintenanceNotification;
```
...
describe('SubscriptionManager Integration', () => {
  ...
  beforeEach(async () => {
    // Clean Redis state
    await redis.flushdb();
    subscriptionManager = new SubscriptionManager(redis);
  });

  it('should handle concurrent subscriptions', async () => {
    // Test real multi-service interaction
    const subscriptions = await Promise.all([
      subscriptionManager.subscribe('channel-1'),
      subscriptionManager.subscribe('channel-2'),
      subscriptionManager.subscribe('channel-3')
    ]);

    expect(subscriptions).toHaveLength(3);
    expect(await redis.scard('active_subscriptions')).toBe(3);
  });
});
```
  ...
  it('should process notifications within acceptable time', async () => {
    ...
    await Promise.all(
      Array.from({ length: 100 }, (_, i) => 
        processNotification({ id: `notif-${i}` })
      )
    );
...
4. **Mock Cleanup**: Always clean up mocks in `afterEach` to prevent test pollution
5. **Async Testing**: Use `async/await` instead of callbacks for clearer async tests
6. **Error Testing**: Test both success paths and error conditions
7. **Edge Cases**: Test boundary conditions and edge cases
8. **Performance**: Include performance tests for critical paths
9. **Documentation**: Comment complex test setups or assertions
